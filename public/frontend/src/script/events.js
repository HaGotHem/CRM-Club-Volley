document.addEventListener('DOMContentLoaded', async () => {
    ...
    async function loadSeasonEvents(season) {
        ...
        try {
            const response = await fetch(`/api/events?season=${encodeURIComponent(season)}&t=${Date.now()}`);
            const result = await response.json();
    ...
    function createCardHtml(event) {
        ...
        return `
            <div class="${cardBg} ${borderCol} border rounded-2xl p-6 transition-all hover:shadow-md flex flex-col md:flex-row md:items-center justify-between gap-6 event-card" data-event-id="${event.id}">
    ...
    function renderSection(container, events) {
        ...
        container.innerHTML = events.map(event => createCardHtml(event)).join('');
    ...
    async function importEvent(id, button) {
        ...
        try {
            ...
            if (result.success) {
                // Feedback visuel immédiat : mettre à jour la card spécifiquement
                try {
                    const checkResponse = await fetch(`/api/events/${id}?t=${Date.now()}`);
                    const checkResult = await checkResponse.json();
                    if (checkResult.success) {
                        const updatedEvent = checkResult.data;
                        // On trouve TOUTES les instances de cette carte (peut être dans "En cours" et dans une saison)
                        const cards = document.querySelectorAll(`.event-card[data-event-id="${id}"]`);
                        cards.forEach(card => {
                            const newHtml = createCardHtml(updatedEvent);
                        ...
                        // Ré-attacher les listeners sur les nouvelles cartes (si jamais il y a des boutons, même si ici c'est sensé être "À jour")
                        // Mais par sécurité on relance loadInitialData après le modal
                    }
                } catch (e) {
                    console.error("Erreur lors de la mise à jour locale de la card", e);
                }
                ...
                // On attend un tout petit peu que la DB se stabilise (latence possible selon l'env)
                setTimeout(async () => {
                    // Recharger TOUT pour être sûr du tri et des sections
                    await loadInitialData();
                ...
                }, 1000); // Augmenté à 1s car on a déjà mis à jour la card visuellement
            } else {
            ...
            // Si le bouton existe encore dans le DOM (il a pu être remplacé par loadInitialData)
